# Weybre AI — Guardrails Reference
...
Disclaimer behavior is controlled by the `WEYBRE_DISCLAIMERS` environment variable:

- **`WEYBRE_DISCLAIMERS=true`** (default): Append the standard disclaimer to every substantive deliverable.
- **`WEYBRE_DISCLAIMERS=false`**: Warnings are displayed on-screen only — never embedded in output documents. Clean documents only.

### Standard Disclaimer (when WEYBRE_DISCLAIMERS=true)
...
```
[Weybre AI] Reminder: This is AI legal research, not legal advice. No attorney-client relationship. Get a licensed attorney to review before acting on anything here.
```
...
*This reference is version 1.0. All agents in the Weybre AI firm MUST comply with every provision. No exceptions. No shortcuts.*
